...
`.issues/` is a git worktree (orphan branch worktree at `.git/worktrees/-issues/`) and is gitignored in the parent repo (`.gitignore` line 40). Agents are ignoring `.gitignore` and tracking `.issues/` files in the parent repo's git, which corrupts git state and breaks branches.
