...
- [ ] 28. **Add critical violation to `000-critical-rules.md` (**sub-agent**).** Open `.opencode/guidelines/000-critical-rules.md`. Find the Tier 2 (process-integrity) section. Append:
...
**→ SC-5**
  - [ ] 28.1. **RED (**sub-agent**).** Write behavioral test that sends prompt with a task card containing `task()` and verifies the agent declines to execute it. Test fails because no prohibition exists.
  - [ ] 28.2. **GREEN (**sub-agent**).** Append the critical violation entry as described above.
...
- [ ] 29. **Verify 13 clean task cards unmodified (**sub-agent**).** Run:
...
If any changes detected, revert them with `git checkout -- <file>`. **→ SC-6**
  - [ ] 29.1. **RED (**sub-agent**).** Write behavioral test that verifies git diff shows zero changes to the 13 clean task cards. Test fails if any changes detected.
  - [ ] 29.2. **GREEN (**sub-agent**).** Run the git diff command. If any changes, revert them.
