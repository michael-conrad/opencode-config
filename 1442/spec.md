...
|----|-----------|---------------|---------------------|-------------|----------------------|--------------|--------------|-----------------|-----------------|
| SC-1 | All 7 auditor task files reject `next_step: proceed` for FAIL criteria — `next_step` MUST be `"remediate"` when `result` is `"FAIL"` | `behavioral` | Run `opencode-cli run` with a prompt that triggers an auditor sub-agent producing a FAIL criterion — `assert_stderr_pattern_present 'next_step.*remediate'` — verify the auditor does NOT produce `next_step: "proceed"` for FAIL results | If auditor produces `proceed` on FAIL, fix template and re-run test | pre-commit | `.opencode/skills/adversarial-audit/tasks/` | Phase 1 | pre-commit | atomic |
| SC-2 | All 7 auditor task files have `all_criteria_pass: bool` in per-criterion YAML template and result contract | `behavioral` | Run `opencode-cli run` with a prompt that triggers an auditor sub-agent — `assert_stderr_pattern_present 'all_criteria_pass'` — verify the auditor result contract includes the field | If missing, add field to template and result contract and re-run test | pre-commit | `.opencode/skills/adversarial-audit/tasks/` | Phase 1 | pre-commit | atomic |
...
| SC-5 | cross-validate.md per-criterion template has `next_step` conditional on PASS/FAIL and includes `all_criteria_pass` | `behavioral` | Run `opencode-cli run` with a prompt that triggers cross-validate sub-agent — `assert_stderr_pattern_present 'all_criteria_pass'` — verify the cross-validate result contract includes the field and conditional next_step | If missing, add to template and re-run test | pre-commit | `.opencode/skills/adversarial-audit/tasks/cross-validate.md` | Phase 1 | pre-commit | atomic |
