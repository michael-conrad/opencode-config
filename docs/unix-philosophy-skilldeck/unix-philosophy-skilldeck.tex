...
These principles were devised for operating-system processes communicating through pipes.  Remarkably, they map onto the emerging field of AI agent architecture with surprising fidelity.  A skill is a ``program'' that does one thing well.  Sub-agents are ``processes'' connected by structured result contracts (``pipes'').  The orchestrator is a ``shell'' that composes them.
...
The Unix philosophy's insight about composing small programs through pipes maps directly to sub-agent decomposition.  The research validates this:
